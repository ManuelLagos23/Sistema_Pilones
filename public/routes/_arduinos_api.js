...
router.get('/', async (req, res) => {
    try {
        const results = await arduinosController.getAllNombrearduinos();
        res.json(results);
    } catch (error) {
        console.error(error);
        res.status(500).json({ error: 'Error obtaining arduinos registers' });
    }
});
...
router.delete('/:arduinoId', async (req, res) => {
    try {
        const arduinoId = req.params.arduinoId;
        console.log("IDEEEEEEEEEEEE" + arduinoId);
